        ...
        <el-row :gutter="20">
            <el-col :xs="24" :sm="24" :md="24" :lg="17" :xl="17">
                ...
                <div class="write_abstract">
                    ...
                    <Editor style="height: 420px; overflow-y: hidden;" v-model="valueHtml" :defaultConfig="editorConfig"
                        :mode="mode" @onCreated="handleCreated" @change="change" class="article" />
                </div>
            </el-col>
            <el-col :xs="24" :sm="24" :md="24" :lg="7" :xl="7">
                <div class="write_abstract">
                    <h4>摘要</h4>
                    <el-input v-model="write.abstractValue" :autosize="{ minRows: 3, maxRows: 5 }" maxlength="200"
        ...
        setup() {
            ...
            const submitAdd = () => {
                ...
                if (state.write.titleValue != "" && state.write.abstractValue != "" &&
                    state.write.classifyValue != "" && state.write.stateValue != "" && state.write.commentValue !=
                    "") {
                    // 添加文章
                    const params = {
                        ...
                        1, // 审核(1待审核，2审核通过，3审核失败) 判断当前用户时普通用户(1)还是管理员(1)
                    ...
                    axios.post("/article/insertArticleInfo", params).then((res) => {
                        if (res.data.code == 0) {
                            ...
                            axios.get("/updateArticleNumberByClassifyId", {
                                ...
                                ElNotification({
                                    title: '提示',
                                    message: state.userInfo.userType == 0 ? "文章发布成功！" :
                                        "文章提交成功，等待管理员审核",
                                    type: 'success',
                                })
                            })
...
<style scoped>
    ...
    .addArticle_box>h3{
        color: #2fa7b9;margin-bottom:10px;padding: 20px 20px;background-color: white;
    }
