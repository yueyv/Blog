                ...
                <el-row :gutter="20">
                    <el-col :xs="24" :sm="24" :md="24" :lg="17" :xl="17">
                        <div class="write_abstract">
                            <h4>标题</h4>
                            <el-input v-model="write.titleValue" placeholder="请输入文章标题" maxlength="50" show-word-limit
                        ...
                        <div class="write_abstract">
                            ...
                            <Editor style="height: 480px; overflow-y: hidden;" v-model="valueHtml"
                    ...
                    <el-col :xs="24" :sm="24" :md="24" :lg="7" :xl="7">
                        <div class="write_abstract">
                            <h4>摘要</h4>
                            <el-input v-model="write.abstractValue" :autosize="{ minRows: 3, maxRows: 5 }"
...
<script>
    ...
    export default {
        ...
        props: [
            'compileArticleDialogFormVisible',
            'valueHtml'
        ],
        ...
        setup(props, {
            ...
            const toolbarConfig = {}
            const editorConfig = {
                placeholder: '在JAVA_work，开始您的Write吧~',
                MENU_CONF: {}
            }
            ...
            editorConfig.MENU_CONF['uploadImage'] = {
                // 上传图片的配置
                server: process.env.VUE_APP_URL + "/articleImgUpload",
                // 单个文件的最大体积限制，默认为 2M
            ...
            editorConfig.MENU_CONF['uploadVideo'] = {
                // 上传视频的配置
                server: process.env.VUE_APP_URL + 'articleVideoUpload',
                // 单个文件的最大体积限制，默认为 20M
            ...
            watch(() => props.valueHtml, (val, preVal) => {
                ...
                if (str.includes('http://localhost:80/')) {
                    str = str.replaceAll('http://localhost:80/', "");
                } else if (str.includes('http://localhost/')) {
                    str = str.replaceAll('http://localhost/', "");
                } else if (str.includes('http://192.168.195.68:80/')) {
                    str = str.replaceAll('http://192.168.195.68:80/', "");
                } else if (str.includes('http://192.168.195.68/')) {
                    str = str.replaceAll('http://192.168.195.68/', "");
                }
            ...
            const handleAvatarSuccess = (response) => {
                state.write.thumbnailValue = response;
            }
            ...
            const submitupdate = () => {
                ...
                if (state.write.abstractValue === state.articleInfo.articleDase &&
                    ...
                    ((process.env.VUE_APP_URL + state.write.thumbnailValue) === state.articleInfo
                        .articleImgLitimg ||
                ...
                // 只修改了文章分类、文章状态、文章评论状态无须等待审核
                else if ((state.write.abstractValue === state.articleInfo.articleDase &&
                        ((process.env.VUE_APP_URL + state.write.thumbnailValue) === state.articleInfo
                            .articleImgLitimg || state.write.thumbnailValue === state.articleInfo
                            .articleImgLitimg) &&
                        state.write.titleValue === state.articleInfo.articleTitle &&
                        valueHtml.value === state.articleInfo.articleContent) &&
                    (parseInt(state.write.stateValue) !== state.articleInfo.articleState ||
                    ...
                    // 判断内容是否填写完整
                    if (state.write.titleValue != "" && state.write.abstractValue != "" &&
                        state.write.classifyValue != "" && state.write.stateValue != "" && state.write
                        .commentValue != "") {

                        // 判断文章分类是否修改  修改了原先分类文章数量-1   现分类文章数量+1
                        if (state.write.classifyValue !== state.articleInfo.articleClassifyId) {
                            // 增加现分类对应的文章数量  +1
                ...
                // 修改了文章其他内容
                    ...
                    if (state.write.titleValue != "" && state.write.abstractValue != "" &&
                        ...
                        const params = {
                            ...
                            articleContent: valueHtml.value, // 文章内容
                            articleState: parseInt(state.write.stateValue), // 文章状态
                            articlePass: state.userInfo.userType == 0 ? 2 :
                            1, // 审核(1待审核，2审核通过，3审核失败) 判断当前用户是普通用户(1)还是管理员(1)
                            commentState: parseInt(state.write.commentValue) // 文章评论状态
                        }
                        axios.post("/article/updateArticleInfo", params).then((res) => {
                            if (res.data.code == 0) {
