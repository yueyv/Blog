        ...
        <div>
            <el-table v-loading="loading" element-loading-text="玩命加载中..." :data="tableData"
                ...
                <el-table-column label="缩略图">
                    <template #default="scope">
                        <el-image style="width:70px" :src="url+scope.row.urlLitimg" fit="cover"
                            preview-teleported="true" :preview-src-list="[url+scope.row.urlLitimg]"
...
<script>
    ...
    function auditScreen(state) {
        ...
        axios.get("/url/page/auditScreening", {
            params
        }).then(res => {
            state.tableData = res.data.data.list;
            state.total = res.data.data.total;
